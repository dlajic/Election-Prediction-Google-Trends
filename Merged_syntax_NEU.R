...
data_models <- expand.grid(election_date = as.Date(c("26-09-2021",
                           ...
                           datasource_weight = c("GT",
                                                 ...
                                                 "Only polls",
                                                 "Last polls"),
                           model_time_interval = duration(seq(7,90, 7)[c(1,4, 8, 10)], "days"),
                           model_time_distance = days(c(1,7,14,30,60,90)), # 1 tag vorher, 3 tage, 7 tage, 14 tage
                           model_time_id = "days")
...
data_plot$model_time_interval_fac <- factor(data_plot$model_time_interval_fac,
                                            levels = c("Interval: 7 days", "Interval: 14 days", "Interval: 21 days", 
                                                       ...
                                                       "Interval: 70 days", "Interval: 77 days", "Interval: 84 days"),
                                            ordered = TRUE)
...
data_plot2 <- data_plot %>%
  ...
  group_by(model_name) %>% 
  mutate(deviation_mean = mean(abs(deviation), na.rm=TRUE))

cols2 <- c("GT" = "red", "Only polls" = "black", "Last polls" = "yellow")
...
ggsave(plot = p2,
       filename = "plot_meanDeviations_GT_polls.pdf", # e.g. change to pdf
       width = 14,
       height = 10,
       device = "pdf", # e.g. change to pdf
       dpi = 300)
...
cols3 <- c("GT" = "red", "Only polls" = "black", "Last polls" = "yellow", "GT + election weight" = "blue", 
           "GT + polls weight" = "orange", "GT + weekly polls weight" = "green")
...
cols4 <- c("GT" = "red", "Only polls" = "black", "Last polls" = "yellow", "GT + weekly polls weight" = "green")
...
cols3 <- c("GT" = "red", "Only polls" = "black", "GT + election weight" = "blue", 
           "GT + polls weight" = "orange", "GT + weekly polls weight" = "green", "Last polls" = "yellow")
