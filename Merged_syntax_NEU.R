...
## delete column for political party "Freie WÃ¤hler" and column "Other"
...
# Merge data_models with data_models_GT
  ...
  names_df <- c("2022-07-17 14-13-13","2022-07-22 21-13-09")
    ...
    for(i in 1:nrow(data_models)){
      ...
      if(str_detect(data_models$datasource_weight[i], "GT")){
        ...
        if(length(name_GT_datasets_i)==1){ # THIS PART FOR 2005/2009
          ...
          print(table(data_models$data_GT_year[[i]]$keyword))
  ...
  #And again renaming since first idÂ´s are missing (deleted 2005Â´s)
  ## Add model index/number ####
  data_models <- data_models %>% 
    mutate(model_id = row_number()) %>%
    select(model_id, everything())
