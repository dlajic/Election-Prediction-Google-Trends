...
```{r filter-non-identical-datasets}
...
names(data_comparisons) <- c("name", "trend_05", 
                             "trend_09", "trend_AFD_13",
                             "trend_AFD_17", "trend_AFD_21", 
                             "trend_CDU_13", "trend_CDU_17",
                             "trend_CDU_21", "trend_sonst_05", 
                             "trend_sonst_09", "trend_sonst_13",
                             "trend_sonst_17", "trend_sonst_21",
                             "trend_05_identical",
                             "trend_09_identical",
...
rm(list=setdiff(ls(), c("data_models", "data_predictions_final", "data_predictions", "data_comparisons2"))) 
```
...
|                                                            |                                                                                                     |                             |                                                     |                                  |                |                      |                                         |
| Note: \* = Non-binary outcome; ? = not enough information; |                                                                                                     |                             |                                                     |                                  |                |                      |                                         |

: Overview of choices in previous studies {#tbl-1, tbl-colwidths="[12.5, 12.5, 12.5, 12.5, 12.5, 12.5, 12.5, 12.5]"}
...
In @fig-4 we visualize a subset of the predictions provided by MC1 models for the most recent general election in Germany (26th of September 2021). @fig-4 plots 3600 deviations = predictions - actual vote shares across parties as points connected by lines, namely predictions for 6 parties of based on 600 models defined by 4 widths and 150 distances.^[In total we have, 8 (widths) * 150 (distances) = 1200 models across. Across the 4 elections these MC1 models yield (1200 * 6 parties * 3 elections) + (1200 * 5 parties* 1 election) =  27600 predictions. Figure 1 displayes a subset namely 4 (widths) * 150 (distances) * 6 (parties) = 3600 predictions across parties. For the 2009 election we predict the vote shares of 5 parties yielding  8 * 150 * 5 = 6000 predictions. For the 2013, 2017, 2021 election we predict vote shares for 6 parties yielding 8 * 150 * 3 * 6 = 21600 predictions. This results in a total of 27600 predictions.] 
The most recent election is ideal for benchmarking GT predictions, because usage of the platform has changed over time. Connecting lines have been added to facilitate comparison. The single panels (Plot 1-4) correspond to the four different widths of the data windows going from 7 days to 91 days (see right-hand y-axis). The x-axis indicates the distance of the respective window to the election. First, we find that the width and distance of the data window to the election matters. We compare models of differing width (from top to bottom) and distance (from left to right). Data windows of low width, e.g., the models based on 7 days of GT data in the top plot, vary much stronger in their predictive accuracy across time. In other words, with such small time spans it matters which week of GT data we have pick for our prediction. Naturally, this variation decreases when we extend the time window on which we base our prediction going from 7 days (Plot 1) to 91 days (Plot 4) at the bottom in @fig-4. Second, on average accuracy seems to improve slightly the smaller the distance between the GT data window and the election. In @fig-3 (next section) we plot the prediction error averaged across all parties for MC1 models (red line) contrasting them with other models. Focusing on the red line this analysis also indicates that the error decreases the closer we are to the election. However, we can also clearly see that picking a short data window, e.g., 7 days (Plot 1), results in considerable variation in what regards accuracy. In addition, we used a linear model to model the trend of accuracy as a function of distance holding the width contant at 14 days in @tbl-A2. For the 2021 election, the average deviation decreases by 0.02% per day distance. In other words, if move the data window 100 days closer to the election, the absolute mean deviation decreases by 2%. Third, @fig-4 also highlights the strong variation in accuracy between parties. For instance, GT predictions of the vote share of the Linke are more accurate than, e.g., for the AFD, with the deviation being closer to 0 across models. For the 2021 election the error generally seems highest for the AfD. Moreover, shares of certain parties are constantly underestimated (e.g., Linke), while others are overestimate (e.g., AfD). It seems that the quality of Google searches as a signal of vote choice, different across parties. Importantly, in Appendix A.4 @fig-A1 provides further visualizations for more intervals for the 2021 election, @fig-A2 provide visualizations for the other elections and Table A1 also provides models for the other elections. The insights regarding data windows described above are largely confirmed when using data for other elections.

## Comparing predictions across model classes

Above, we compared different models within class MC1 (only GT data) focusing on the width and distance of data windows. Previous studies have often combined GT data with election data or polls. As described in Section 3 we compare predictions across three broad classes of models. In @fig-5 again focuses on the 21' election. Now, each point represents the mean absolute error, i.e., the prediction error averaged across parties whereby predictions by different models are colored: MC1 is solely based on GT data (red line, 600 models), MC2 is weights GT data with previous election results (purple line, 600 models), MC3 weights GT data with poll data (orange line, 600 models). Finally, we have the pure polling data, namely always the last Dimap polls in the respective time windows (black line, 600 models). The various plots (Plot 1-4) in @fig-5 corresponding to different widths of the data window.
...
No surprisingly, we find that polls (black line) unarguably perform the best for the 2021 election reflected in the fact that prediction error mostly scores below our models of Class MC1-3. In @fig-3 this becomes visible as the black line is almost always closer to 0 then the other ones. We also compared how often other models actually provide better predictions. The next best model class, MC3, combines GT data with polls. Across all models visualized in @fig-3, in 24.5% (147 out of 600) of predictions the MC3 models beat the polls in terms of accuracy. In other words, sometimes the combination of the two data sources provides a more accurate prediction. MC1 models perform worse and only provide a better predictions in 5.8% (35 out of 600) cases. Finally, models of class MC2 that combine GT data with previous election perform even worse and only provide better predictions than polls in 4.5% (27 out of 600) of cases. Hence, our data suggests that weighting GT data with previous elections is generally unhelpful. Naturally, we could discuss whether the accuracy of our GT based models (MC1) is really so much worse than polling data. Given that we can collect GT data for free, errors of the size found here may be acceptable for certain applications where we only need rough predictions. Besides, if we want to decrease the dependency of GT-based predictions on short-term trends we should probably base our predictions on a larger time span as shown in Plot in @fig-3. We provide findings for other elections in the next section and in Appendix A.4.2.

## Comparing predictions across model classes and elections

Above we focussed on the most recent election in Germany (September 2021). This focus is justified because, given the changing user population, the most recent election is the most appropriate to assess the predictive power of the GT search data. Nonetheless, a comparison with earlier elections can provide us with some insights as to what variations we can expect for coming elections. @fig-6 is similar to @fig-5 only that the width of the data window is held constant at 91 days - corresponding to the shown in the lowest panel of @fig-5 - and predictions for different elections are shown on top of each other. Again, each point represents the mean absolute error, i.e., the prediction error averaged across parties whereby predictions by different model classes are colored. We find that indeed the accuracy of predictions varies across election. To start GT based predictions (MC1: GT, red lines) were least accurate in 2009 (@fig-6, Plot 2) and 2021 (@fig-6, Plot 3), never beating the polls in terms of accuracy. However, MC1 models fared better in 2013, beating polls in 66% (99 out of 150) predictions and 2017 with 97% (145 out of 150) predictions (cf. @fig-6, Plot 2 and 3). MC2 models (GT + election weight) only provide better predictions in 2017 in 45% of cases (68 out of 150). MC3 exceeded polls in terms of accuracy in 2009 for 30 our of 150 predictions (20%), in 2013 for 43 our of 150 predictions (29%), in 2017 for 109 our of 150 predictions (73%) and in 2021 for 30 our of 150 predictions (20%). Hence, in general, 2017 was a particularly bad year for the polls in our data. We can only speculate why there is such strong variation across elections. First, as mentioned, we have to assume that the representativity of Google search users varies across time. In principle, it is possible that the population of Google search users was more representative of German voters in the 2013, 2017 elections than in 2009 or 2021. Second, it could be related to the elections themselves. Potentially, the extend to which the action "search for" is related to "vote for" depends on the election with more "charged" elections like the 2021 election decreasing this correlation (Faas and Klingelhöfer 2022). Third, it is possible that the accuracy of the polling data which we use as a benchmark varies over time.
...
@fig-A3: Sampling error variation for the 2021 election
...
```{r eval=FALSE}
#| label: tbl-A2
#| tbl-cap: "Prediction accuracy as a function of distance"
#| tbl-colwidths: auto
...
### Comparing predictions across model classes: Other elections and data window widths
...
```{r}
#| label: fig-13
#| fig-cap: "Benchmarking against other polls"
#| fig-width: 14
#| fig-height: 14
#| fig-pos: "H"
...
# Setup of data collection and replication information

* Step 1: Collect GT Data
* Step 2: Collect polling data
