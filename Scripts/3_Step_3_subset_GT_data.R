...
# Dataframe
  ...
  names(data_comparisons) <- c("name", "trend_05", 
                               "trend_09", 
                               "trend_AFD_13",
                               "trend_AFD_17", 
                               "trend_AFD_21", 
                               ...
                               "trend_05_identical",
                               "trend_09_identical",
                               "trend_AFD_13_identical", 
                               "trend_AFD_17_identical",
                               "trend_AFD_21_identical",
                               "trend_CDU_13_identical", 
                               "trend_CDU_17_identical", 
                               "trend_CDU_21_identical", 
                               "trend_sonst_05_identical", 
                               "trend_sonst_09_identical", 
                               "trend_sonst_13_identical", 
                               "trend_sonst_17_identical", 
...
# Dataset list for comparison ####
  # Create list of datasets for comparison
  for (y in 1:length(df)){ # 
    #y <- 1
    load(df[y])
    objects <- ls()[str_detect(ls(), "trend")]
    objects <- objects[objects!="trend_all_cat" & objects!="trend_all_cat_AFD"]
...
# Compare datasets ####
  # Compare in Rdata with those of Rdata in previous row
...
# Copy suitable datasets ####
  # Copy datasets from "Data_raw" to "Data" (these are used for analysis)
  unlink("../Data", recursive=TRUE)
  dir.create("../Data/Data_GT", recursive = TRUE, showWarnings = FALSE)
  for(i in datasets_we_can_use){
    #i <- "2022-12-07 04-31-13.RData"
    file.copy(from = paste0("../Data/Data_raw/", i),
              to = paste0("../Data/Data_GT/", i),
              overwrite = TRUE)
  }
