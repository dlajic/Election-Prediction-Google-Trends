...
names_df <- list.files(dir)  
  ...
  x <- 1:36000
  x <- x[str_detect(data_models$datasource_weight, "GT")]
  ...
  for(y in names_df){
    ...
    for(i in 1:500){ # nrow(data_models)
      # Load GT datasets
      print(i)
      name <- y
      load(name)
      ...
      # if(str_detect(data_models$datasource_weight[i], "GT")){
        ...
        if(length(name_GT_datasets_i)==1){ # THIS PART FOR 2005/2009
          ...
          data_models$data_GT_year[[i]] <- get(name_GT_datasets_i[1])$interest_over_time %>% # Ony 1 dataset
            select(date, hits, keyword) %>%
            mutate(hits = str_replace(hits, "<1", "0"), 
                   hits = as.numeric(hits), 
                   date = as.Date(date))%>% 
          ...
          df1 <- get(name_GT_datasets_i[1])$interest_over_time %>% select(date, hits, keyword) %>%
            mutate(hits = str_replace(hits, "<1", "0"), # HIER WEITER
                   hits = as.numeric(hits), 
                   date = as.Date(date))%>% 
            replace(is.na(.), 0) %>%
    ...
    end_time <- Sys.time()
    end_time - start_time 
    # Tested for 500 models/rows
    # Normal time: 55.8 sec
    # 52.54618 secs when directly assigning df1 and df2 (keine Zwischenspeicherung) -> save 3 seconds (*36000!)
    # 49.72478 secs when deleting all the print commands
    # directly inserting get() into bind_rows slows the loop!
    # Do we need all data in df1 and df2? or can we subset? -> I THINK YES
    # Initiating list before does not help..   data_models$data_GT_year <- list(NA)
    # Create index beforehand to only loop over 24000 out of 36000 models
    # Write stuff to list instead of df? Maybe faster? Test it tomorrow!
