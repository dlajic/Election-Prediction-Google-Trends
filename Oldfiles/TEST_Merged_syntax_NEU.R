...
setwd("C:/Users/deanl/Desktop/UniMannheim/ComSocScience/Publikation/Election-Prediction-Google-Trends/Workspace")
...
## delete column for political party "Freie WÃÂÃÂ¤hler" and column "Other"
...
names_df <- list.files(dir)  
  ...
  #And again renaming since first idÂ´s are missing (deleted 2005ÃÂÃÂ´s)
  ## Add model index/number ####
  data_models <- data_models %>% 
    mutate(model_id = row_number()) %>%
    select(model_id, everything())
