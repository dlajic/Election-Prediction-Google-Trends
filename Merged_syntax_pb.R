...
list_electionresults[["2005-09-18"]] <- data.frame(party=c("CDU", "FDP", "Grne", "Linke", "SPD"),
                                                   share=c(35.2, 9.8, 8.1, 8.7, 34.2))

list_electionresults[["2009-09-27"]] <- data.frame(party=c("CDU", "FDP", "Grne", "Linke", "SPD"),
                                                   share=c(33.8, 14.6, 10.7, 11.9, 23)) %>% arrange(party)

list_electionresults[["2013-09-22"]] <- data.frame(party=c("AFD", "CDU", "FDP", "Grne", "Linke", "SPD"),
                                                   share=c(4.7, 41.5, 4.8, 8.4, 8.6, 25.7)) %>% arrange(party)

list_electionresults[["2017-09-24"]] <- data.frame(party=c("AFD", "CDU", "FDP", "Grne", "Linke", "SPD"),
                                                   share=c(12.6, 32.9, 10.7, 8.9 , 9.2, 20.5)) %>% arrange(party)

list_electionresults[["2021-09-26"]] <- data.frame(party=c("AFD", "CDU", "FDP", "Grne", "Linke", "SPD"),
...
#colnames(infra_dimap_all) <- c("Date", "SPD", "CDU", "Grne", "FDP", "AFD", "Linke")
...
data_models <- data_models %>%
  mutate(parties = ifelse(election_date <="2009-09-27",
                          list(c("CDU", "SPD", "Grne", "Linke", "FDP")),
                          list(c("CDU", "SPD", "Grne", "Linke", "FDP", "AFD"))
  ))
...
data_models$GT_keywords[grepl("GT", data_models$model_name) & grepl("M_\\d+_2005", data_models$model_name)] <- list(c('CDU + CSU + Angela Merkel', 'SPD + Gerhard Schröder', 'Grne + Joschka Fischer', 'PDS + Linkspartei + Gregor Gysi',
                                                                                                                      'FDP + Guido Westerwelle'))
data_models$GT_keywords[grepl("GT", data_models$model_name) & grepl("M_\\d+_2009", data_models$model_name)] <- list(c('CDU + CSU + Angela Merkel', 'SPD + Frank Walter Steinmeier', 'Grne + Jrgen Trittin + Renate Knast', 'Linke + Gregor Gysi',
                                                                                                                      'FDP + Guido Westerwelle')) #
data_models$GT_keywords[grepl("GT", data_models$model_name) & grepl("M_\\d+_2013", data_models$model_name)] <- list(list(c('CDU + CSU + Angela Merkel', 'SPD + Peer Steinbrck', 'Jrgen Trittin + Katrin Göring Eckardt + Grne + ', 'Linke + Sarah Wagenknecht + Gregor Gysi',
                                                                                                                           'FDP + Philipp Rösler'),
                                                                                                                         c('Bernd Lucke + Afd', 'CDU + CSU + Angela Merkel', 'SPD + Peer Steinbrck', 'Jrgen Trittin + Katrin Göring Eckardt + Grne',
                                                                                                                           'FDP + Philipp Rösler')))
data_models$GT_keywords[grepl("GT", data_models$model_name) & grepl("M_\\d+_2017", data_models$model_name)] <- list(list(c('CDU + CSU + Angela Merkel', 'SPD + Martin Schulz', 'Cem Özdemir + Katrin Göring Eckardt + Grne', 'Linke + Sarah Wagenknecht + Dietmar Bartsch',
                                                                                                                           'FDP + Christian Lindner'),
                                                                                                                         c('Alice Weidel + Alexander Gauland + Afd', 'CDU + CSU + Angela Merkel', 'SPD + Martin Schulz', 'Cem Özdemir + Katrin Göring Eckardt + Grne',
                                                                                                                           'FDP + Christian Lindner')))
data_models$GT_keywords[grepl("GT", data_models$model_name) & grepl("M_\\d+_2021", data_models$model_name)] <- list(list(c('CDU + CSU + Armin Laschet', 'SPD + Olaf Scholz', 'Annalena Baerbock + Grne', 'Linke + Janine Wissler + Dietmar Bartsch',
                                                                                                                           'FDP + Christian Lindner'),
                                                                                                                         c('Alice Weidel + Tino Chrupalla + Afd', 'CDU + CSU + Armin Laschet', 'SPD + Olaf Scholz', 'Annalena Baerbock + Grne',
                                                                                                                           'FDP + Christian Lindner')))
...
for(i in 1:nrow(data_models)){
  ...
  if(!grepl("M_\\d+_2005", data_models$model_name[[i]]) == TRUE){
    ...
    if(grepl("election weight", data_models$datasource_weight[[i]]) == TRUE){ # Filter GT ONLY
      ...
      if(grepl("M_\\d+_2013", data_models$model_name[[12013]]) == TRUE){
...
data_models <- data_models %>%
  filter(grepl("M_\\d+_2005", model_name) == FALSE) %>% # delete 2005 rows
...
data_predictions <- data_predictions %>%
  filter(grepl("M_\\d+_2005", model_name) == FALSE) %>%
...
data_plot2 <- data_plot %>%
  group_by(model_name) %>% 
  summarise(deviation_mean = mean(deviation, na.rm=TRUE)) # STRANGE THAT IT IS CONSTANT.. ERRORü
...
cols <- c("SPD" = "red", "CDU" = "black", "AFD" = "blue", 
          "FDP" = "orange", "Link" = "pink", "Grne" = "green")
...
ggplot(data = data_predictions,
  ...
  scale_fill_manual(values = c("AFD" = "deepskyblue1", "CDU" = "black", "FDP" = "yellow2","Grne" = "green3", "Linke" = "purple", "SPD" = "red"),
                    name = "Political party",
                    breaks = c("CDU","SPD","AFD","FDP", "Linke", "Grne" )) +
  labs(y="Percentage", x="") +
  theme_minimal() +
