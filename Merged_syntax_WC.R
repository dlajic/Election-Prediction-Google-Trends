...
setwd("C:/Users/deanl/Desktop/UniMannheim/ComSocScience/Publikation/Election-Prediction-Google-Trends")
...
## delete column for political party "Freie WÃÂ¤hler" and column "Other"
...
data_models <- expand.grid(election_date = as.Date(c("26-09-2021",
                           ...
                           datasource_weight = c("GT",
                                                 ...
                                                 "Only polls",
                                                 "Last polls"),
                           #model_time_interval = duration(seq(7,14, 7)[c(1:2)], "days"),
                           #model_time_distance = days(seq(1, 35, 7)), # 1 tag vorher, 3 tage, 7 tage, 14 tage # 1 tag vorher, 3 tage, 7 tage, 14 tage
                           model_time_interval = duration(seq(7,91, 7)[c(1:4, 6, 8, 10, 13)], "days"),
                           model_time_distance = days(seq(1, 150, 7)), # 1 tag vorher, 3 tage, 7 tage, 14 tage # 1 tag vorher, 3 tage, 7 tage, 14 tage
                           model_time_id = "days")
    ...
    if(data_models$datasource_weight[i] == "GT + weekly polls weight"){
      ...
      if(nrow(test) == 0){
        ...
        if (grepl("M_\\d+_2009", data_models$model_name[i])){
          ...
          # Search for the two polls that lie before our interval (first poll (b) to specify the Google data that will be used to calculate the weighting factor with the second poll (k), which will then be used to weight the Google data in our specified interval).
          ...
          # now get Google data 1 day before first poll (k) until the end of our interval 
...
data_plot_WC <- data_predictions_final_mean %>%
...
ggsave(plot = p3,
       filename = "WC_plot_meanDeviations_allModels.pdf", # e.g. change to pdf
       width = 14,
       height = 10,
       device = "pdf", # e.g. change to pdf
       dpi = 300)
...
saveRDS(data_plot_WC, file="data_plot_WC.RDS")
