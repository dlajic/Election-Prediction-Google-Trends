...
# Create variable containing parties that run in particular election
data_models <- data_models %>%
...
# Create variable containing elections results
# 2009
...
for(i in names(list_electionresults)){
  ...
  data_models$data_election_results[data_models$election_date==i] <-
    list(list_electionresults[[i]])
    
}

# Create variable containing previous election results
for(i in 1:length(list_electionresults)){
  
  data_models$data_election_results_previous[data_models$election_date==names(list_electionresults)[i+1]] <-
    list(list_electionresults[[i]])
...
# Add names of dataframe to store predictions results
data_models <- data_models %>%
...
# Create variable that contains datasets relevant for model(s)
...
data_models$name_GT_datasets[grepl("GT", data_models$model_name) & grepl("2021", data_models$model_name)] <- list(c("trend_CDU_21",
                                                                                                                 "trend_AFD_21"))
# Create variable that contains election results per year
  ...
  if(data_models$datasource_weight[i]=="GT + election weight"){ # Filter GT ONLY
    ...
    data_models$predictions_GT_election_weight[[i]] <- 
      left_join(data_models$predictions_GT[[i]], 
                data_models$data_election_results_previous[[i]], 
                by= "party") %>%
      mutate(prediction = rowMeans(select(.,prediction, share))) %>%
      select(party, prediction)
  }} 
...
# Pull together all predictions  
  data_predictions <- data_models %>% 
    select(model_id, model_name, election_date, datasource_weight,
           ...
           data_election_results, predictions) %>%
    unnest(cols = c(data_election_results, predictions))
...
for(i in counter){
  ...
  df_AFD_13 <- df_AFD_13 %>%
    filter(grepl("Afd.*", df_AFD_13$keyword) == TRUE) # CHECK: REFER TO DATASET?
